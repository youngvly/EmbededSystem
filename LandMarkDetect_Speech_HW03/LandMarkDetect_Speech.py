...
def detect_landmarks(path,script):
    ...
    #landmarkList.append(landmarks[0].description)
    if script in landmarks[0].description:
        img = cv2.imread(path)
...
def imageEqual(script) :
    ...
    #landmarkList.clear()
    for f in filelist :
        newthread = threading.Thread(target = detect_landmarks,args= (basePath + f,script))
        newthread.start()
